    ...
    const filteredAndSortedProductos = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(prod =>
                prod.name.toLowerCase().includes(lowSearch) ||
                prod.id.toLowerCase().includes(lowSearch)
            );
        }
        ...
        <div className="space-y-8 pb-12 animate-in fade-in duration-700">
            <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-6 bg-white dark:bg-slate-900 p-8 md:p-10 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] border border-slate-100 dark:border-slate-800 transition-colors duration-300">
                <div className="space-y-3">
                    ...
                    <div className="mt-4 px-4 py-3 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-xl inline-flex items-center gap-3">
                    ...
                    <div key={prod.id} className="group flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div className="relative aspect-square bg-slate-100 dark:bg-slate-800/50 flex items-center justify-center p-6">
                            ...
                            <div className="absolute top-4 left-4">
                                <span className={`inline-flex items-center px-3 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-sm ${prod.gender === 'Femenino' ? 'bg-pink-100 text-pink-700 dark:bg-pink-500/20 dark:text-pink-400 ring-1 ring-pink-500/20' :
                                    prod.gender === 'Masculino' ? 'bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:text-sky-400 ring-1 ring-sky-500/20' :
                                        prod.gender === 'Unisex' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 ring-1 ring-emerald-500/20' :
                                            'bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-300 ring-1 ring-slate-500/20'
                                    }`}>
                                    {prod.gender}
                                </span>
                            </div>
                                                ...
                                                <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 animate-in slide-in-from-bottom-2">
                                                    ...
                                                    <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                                                        <div className="flex items-center bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 p-1">
                                                            ...
                                                            <button
                                                                onClick={() => updateCartQuantity(item.producto.id, item.priceType, item.quantity + 1)}
                                                                className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-colors font-bold"
                                                            >+</button>
                                                        </div>
                                                        <p className="font-black text-indigo-600 dark:text-indigo-400 min-w-[100px] text-right">
